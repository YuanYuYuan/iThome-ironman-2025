...
pub fn generic_runtime_param(input: TokenStream) -> TokenStream {
...
#[proc_macro_derive(RegisterParam, attributes(param))]
pub fn register_param(input: TokenStream) -> TokenStream {
    let input = syn::parse_macro_input!(input as DeriveInput);
...
) -> (Vec<&Ident>, Vec<TokenStream2>, Vec<TokenStream2>) {
...
fn generate_declare_param(
    ...
    quote! {
        ...
        struct AbstractRuntimeParam {
            #(
                #[serde(default)]
                #aliases: #helper_name<#default_param_of_variant>,
            )*
        }
...
fn derive_register_param(input: DeriveInput) -> Result<TokenStream2, syn::Error> {
    ...
    let tokens = quote! {
        ...
        lazy_static::lazy_static! {
            pub static ref ZENOH_RUNTIME_ENV_STRING: String = std::env::var(ZENOH_RUNTIME_ENV).unwrap_or("()".to_string());
        ...
        impl std::borrow::Borrow<#meta_param> for #enum_name {
...
fn derive_generic_runtime_param(input: DeriveInput) -> Result<TokenStream2, syn::Error> {
